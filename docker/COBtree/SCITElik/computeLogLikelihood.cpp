...
int* getParentVectorFromInput(char* inputTREE, int n);
...
int m = 1000;                // number of samples
char scoreType = 'm';
int rep = 1;            // number of repetitions of the MCMC
int loops = 100;          // number of loops within a MCMC
double gamma = 1;
double fd = 0.02;          // rate of false discoveries (false positives 0->1)
double ad1 = 0.2;          // rate of allelic dropout (false negatives 1->0)
...
char treeType = 'm';        // the default tree is a mutation tree; other option is 'transpose' for (transposed case), where we have a binary leaf-labeled tree
int maxTreeListSize = -1;  // defines the maximum size of the list of optimal trees, default -1 means no restriction

//char parentVectorInputTREE[] = "35,32,27,6,62,84,10,67,95,100,62,63,73,58,78,55,5,93,99,61,27,10,17,53,55,75,69,11,36,78,101,80,78,80,71,3,22,29,74,31,88,95,60,38,36,91,85,27,43,78,90,97,19,38,61,35,2,48,77,26,21,4,96,34,47,58,33,38,101,64,45,59,87,42,40,59,3,46,46,99,90,28,55,69,91,9,77,59,11,20,6,24,46,14,75,75,87,5,31,80";
char* parentVectorInputTREE;
...
int main(int argc, char* argv[])
{
	...
	currTreeParentVec = getParentVectorFromInput(parentVectorInputTREE, n);

	double** currLogScores = getLogScores(errorRates[0], errorRates[1], errorRates[2], errorRates[3]);
	double currTreeLogScore = currTreeLogScore = scoreTreeAccurate( n, m, currLogScores, dataMatrix, scoreType, currTreeParentVec);

	cout << "Log Likelihood:" << currTreeLogScore << "\n";
...
int* getParentVectorFromInput(char* inputTREE, int n){
    ...
    char* token = strtok(inputTREE, ",");
    // loop through the string to extract all other tokens
    int i = 0;
    while( token != NULL ) {
        parentVector[i] = (atoi(token) - 1); //printing each token
        token = strtok(NULL, ",");
        i++;
    }
...
int readParameters(int argc, char* argv[]){
	for (int i = 1; i < argc; ++i) {
		...
		if (strcmp(argv[i], "-i") == 0) {
			...
			if (i + 1 < argc) { parentVectorInputTREE = argv[++i];}
